...
export function engineResearch(store){
 ...
 return{
  ...
  diagnostics:{...(persisted?.diagnostics??{totalSignals:signals.length,tradableSignals:signals.filter(s=>s.action==='SHADOW_BUY'&&(s.reasons??[]).length===0).length,rejectedSignals:signals.filter(s=>s.action!=='SHADOW_BUY'||(s.reasons??[]).length>0).length}),activePolicy:ACTIVE_ENGINE_POLICY,persistedPolicy},recentSignals:recent,
