...
async function settle(ctx,now,st){
 ...
 ctx.s.entity('experiment','temporal-consensus-leaderboard',{at:now,policy:POLICY,shadowOnly:true,researchOnly:true,temporalTtlMs:TEMPORAL_TTL_MS,minIndependentEngines:MIN_ENGINES,entryPolicy:'first-consensus-without-price-momentum',settlementDelayLimitMs:SETTLEMENT_DELAY_LIMIT_MS,settlementSignalLimit:SETTLEMENT_SIGNAL_LIMIT,researchNotionalUsdc:RESEARCH_NOTIONAL_USDC_RAW/1e6,costModel:'jupiter-executable-min-output-roundtrip',promotionPolicy:PROMOTION,signals:settled.length,promotionEligible:rows.filter(r=>r.promotion?.eligible),rows:rows.sort((a,b)=>(b.lower95??-Infinity)-(a.lower95??-Infinity))});
}
export function installMultiEngine(Trader){
 ...
 Trader.prototype.evaluate=async function(){
  ...
  const recent=this.s.entities('engine_signal',5000).filter(s=>s.at>=now-TEMPORAL_TTL_MS&&s.evidence?.prefilteredSafeUniverse===true),mints=unique(recent.map(s=>s.mint));let candidates=0,confirmed=0,routerRejected=0,entryQuoteRejected=0,repeatSuppressed=0,freshRiskRejected=0,momentumExcluded=0;
  for(const mint of mints){
   const sources=sourceSignals(this,mint,now),safe=sources.filter(s=>(s.evidence?.risk?.reasons??[]).length===0),engineNames=unique(safe.flatMap(s=>(s.engines??[]).map(e=>e.engine))),researchEngines=engineNames.filter(x=>x!=='price_momentum');
   if(!safe.length)continue;candidates++;if(researchEngines.length<MIN_ENGINES)continue;confirmed++;if(engineNames.includes('price_momentum'))momentumExcluded++;
   ...
   const signal={id,mint,symbol:newest.symbol,at:now,score:Math.round(mean(safe.map(s=>Number(s.score)||0))),action:reasons.length?'REJECT':'SHADOW_BUY',reasons,policy:POLICY,entryPrice:market?.price??null,entryFill:executable?.fill??null,engines:researchEngines.map(engine=>({engine})),evidence:{temporalConsensus:true,firstConsensusAfterReset:true,entryPolicy:'first-consensus-without-price-momentum',priceMomentumObserved:engineNames.includes('price_momentum'),temporalTtlMs:TEMPORAL_TTL_MS,sourceSignalIds:safe.map(s=>s.id),sourcePolicies:unique(safe.map(s=>s.policy)),engineNames:researchEngines,allObservedEngineNames:engineNames,risk:freshRisk??newest.evidence?.risk,strategy:st,shadowOnly:true,researchCostModel:'jupiter-executable-min-output-roundtrip'}};
  ...
  this.s.entity('experiment','temporal-consensus-funnel',{at:now,policy:POLICY,entryPolicy:'first-consensus-without-price-momentum',temporalTtlMs:TEMPORAL_TTL_MS,mintsWithRecentSafeEvidence:mints.length,candidates,consensusPassed:confirmed,momentumExcluded,repeatSuppressed,freshRiskRejected,routerRejected,entryQuoteRejected,shadowOnly:true});
 };
}
