...
async function settle(ctx,now,st){
 ...
 ctx.s.entity('experiment','temporal-consensus-leaderboard',{at:now,policy:POLICY,shadowOnly:true,researchOnly:true,temporalTtlMs:TEMPORAL_TTL_MS,minIndependentEngines:MIN_ENGINES,settlementDelayLimitMs:SETTLEMENT_DELAY_LIMIT_MS,researchNotionalUsdc:RESEARCH_NOTIONAL_USDC_RAW/1e6,costModel:'jupiter-executable-min-output-roundtrip',promotionPolicy:PROMOTION,signals:settled.length,promotionEligible:rows.filter(r=>r.promotion?.eligible),rows:rows.sort((a,b)=>(b.lower95??-Infinity)-(a.lower95??-Infinity))});
}
export function installMultiEngine(Trader){
 ...
 Trader.prototype.evaluate=async function(){
  ...
  const recent=this.s.entities('engine_signal',5000).filter(s=>s.at>=now-TEMPORAL_TTL_MS&&s.evidence?.prefilteredSafeUniverse===true),mints=unique(recent.map(s=>s.mint));let candidates=0,confirmed=0,routerRejected=0,entryQuoteRejected=0;
  for(const mint of mints){
   ...
   const signal={id,mint,symbol:newest.symbol,at:now,score:Math.round(mean(safe.map(s=>Number(s.score)||0))),action:reasons.length?'REJECT':'SHADOW_BUY',reasons,policy:POLICY,entryPrice:market?.price??null,entryFill:executable?.fill??null,engines:engineNames.map(engine=>({engine})),evidence:{temporalConsensus:true,temporalTtlMs:TEMPORAL_TTL_MS,sourceSignalIds:safe.map(s=>s.id),sourcePolicies:unique(safe.map(s=>s.policy)),engineNames,risk:newest.evidence?.risk,strategy:st,shadowOnly:true,researchCostModel:'jupiter-executable-min-output-roundtrip'}};
   this.s.tx(()=>{this.s.entity('signal',id,signal);this.s.entity('temporal_engine_signal',id,{...signal,outcomes:{}});this.s.event(reasons.length?'TEMPORAL_SIGNAL_REJECTED':'TEMPORAL_SIGNAL_SHADOW',signal,id);this.s.set(id,true)});
  }
  this.s.entity('experiment','temporal-consensus-funnel',{at:now,policy:POLICY,temporalTtlMs:TEMPORAL_TTL_MS,mintsWithRecentSafeEvidence:mints.length,candidates,consensusPassed:confirmed,routerRejected,entryQuoteRejected,shadowOnly:true});
 };
}
