...
const PERSISTENCE_CONCURRENCY = 2;
...
export class Store {
 ...
 PRAGMA user_version=1;`);
  ...
  hydrate(data) { const now=Date.now(),cutoff=now-ACTIVE_JOB_MAX_AGE_MS; this.tx(() => {
    ...
    for (const r of data.jobs) { const stale=r.state==='pending'&&r.available<cutoff; this.db.prepare("INSERT OR REPLACE INTO jobs(id,body,state,attempts,available,lease,lease_until) VALUES(?,?,?,?,?,?,?)").run(r.id,r.body,stale?'dead':r.state,r.attempts,r.available,stale?null:r.lease,stale?null:r.lease_until); }
  ...
  set(key,value){const encoded=JSON.stringify(value),prior=this.db.prepare("SELECT value FROM meta WHERE key=?").get(key)?.value;if(prior===encoded)return;this.db.prepare("INSERT INTO meta VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").run(key,encoded);this.mirror(()=>this.persistence.upsert("meta",{key,value:encoded},"key"));}
  ...
  backlog(now=Date.now()){const cutoff=now-ACTIVE_JOB_MAX_AGE_MS,r=this.db.prepare("SELECT COUNT(*) AS total, MIN(available) AS oldest, SUM(CASE WHEN state='pending' AND available>=? THEN 1 ELSE 0 END) AS fresh FROM jobs WHERE state IN ('pending','running')").get(cutoff);return{total:Number(r?.total??0),fresh:Number(r?.fresh??0),oldestAgeMs:r?.oldest?Math.max(0,now-r.oldest):0,maxActiveAgeMs:ACTIVE_JOB_MAX_AGE_MS}}
  pruneBacklog(maxAgeMs=ACTIVE_JOB_MAX_AGE_MS,now=Date.now()){const cutoff=now-maxAgeMs;const rows=this.db.prepare("SELECT id FROM jobs WHERE state='pending' AND available<?").all(cutoff);if(!rows.length)return 0;this.db.prepare("UPDATE jobs SET state='dead',lease=NULL,lease_until=NULL WHERE state='pending' AND available<?").run(cutoff);return rows.length}
  close(){this.db.close()}
}
