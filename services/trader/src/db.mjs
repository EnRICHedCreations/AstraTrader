...
export class Store {
 ...
 PRAGMA user_version=1;`);
  ...
  backlog(now=Date.now()){const r=this.db.prepare("SELECT COUNT(*) AS total, MIN(available) AS oldest FROM jobs WHERE state IN ('pending','running')").get();return{total:Number(r?.total??0),oldestAgeMs:r?.oldest?Math.max(0,now-r.oldest):0}}
  pruneBacklog(maxAgeMs=300000,now=Date.now()){const cutoff=now-maxAgeMs;const rows=this.db.prepare("SELECT id FROM jobs WHERE state='pending' AND available<?").all(cutoff);if(!rows.length)return 0;this.db.prepare("UPDATE jobs SET state='dead',lease=NULL,lease_until=NULL WHERE state='pending' AND available<?").run(cutoff);for(const {id} of rows)this.mirror(()=>this.persistence.upsert("jobs",{...this.db.prepare("SELECT * FROM jobs WHERE id=?").get(id)},"id"));return rows.length}
  close(){this.db.close()}
}
