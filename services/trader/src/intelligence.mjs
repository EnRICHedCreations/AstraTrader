...
export function mintProgram(programId) {
  if (programId === TOKEN) return { id: programId, name: "spl-token", recognized: true, executionSupported: true };
  if (programId === TOKEN_2022) return { id: programId, name: "spl-token-2022", recognized: true, executionSupported: false };
  return { id: programId ?? null, name: "unknown", recognized: false, executionSupported: false };
}
...
export function walletScore(observations,asOf=Date.now()){
  const s=strategy(),inventory=new Map(),realizedExits=[];let realized=0,tainted=0,unscoredHistoricalExits=0;
  const rows=observations.filter(t=>t.at<=asOf).sort((a,b)=>a.at-b.at||a.slot-b.slot||a.id.localeCompare(b.id));
  for(const t of rows){
    let p=inventory.get(t.mint)??{lots:[],tainted:false,established:false};
    if(t.side==="transfer"){
      const delta=BigInt(t.rawQty);if(delta===0n)continue;
      p={lots:[],tainted:true,established:true};tainted++;inventory.set(t.mint,p);continue;
    }
    const qty=BigInt(t.rawQty);if(qty<=0n)continue;const usd=Number(t.usdRaw)/1e6;if(!Number.isFinite(usd)||usd<=0)continue;
    if(t.side==="buy"){
      const cost=usd+.02;
      if(p.tainted)p={lots:[],tainted:false,established:false};
      p.established=true;p.lots.push({qty,cost,openedAt:t.at});inventory.set(t.mint,p);continue;
    }
    if(t.side!=="sell")continue;
    const available=p.lots.reduce((n,l)=>n+l.qty,0n);
    if(!p.established&&!p.tainted&&available===0n){unscoredHistoricalExits++;continue;}
    if(p.tainted||available<qty){p={lots:[],tainted:true,established:true};tainted++;inventory.set(t.mint,p);continue;}
  ...
  return{score,roundTrips:n,pnl:realized,wins:wins.length,losses:losses.length,confidence:Math.min(1,n/s.walletConfidenceTrips),expectancy:mean,lowerMean95:lower,profitFactor:losses.length&&losses.some(r=>r.pnl<0)?wins.reduce((x,r)=>x+r.pnl,0)/-losses.reduce((x,r)=>x+r.pnl,0):null,taintedInventoryEvents:tainted,unscoredHistoricalExits,activeTaintedMints:activeTaints,eligible,copyability:"Realized finalized Jupiter exits scored with FIFO cost basis; prefix-only historical exits are unscored until a clean buy establishes cost basis; verified separately with delayed router quotes",asOf}
}
