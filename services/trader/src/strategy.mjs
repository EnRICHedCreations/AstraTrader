...
function changed(){const snapshot=strategy();for(const listener of listeners)listener(snapshot)}
export function onStrategyChange(listener){if(typeof listener!=='function')throw Error('Strategy listener must be a function');listeners.add(listener);return()=>listeners.delete(listener)}
export function setStrategy(value){active=validateStrategy(value,DEFAULTS);changed();return strategy()}
export function patchStrategy(patch){active=validateStrategy(patch,active);changed();return strategy()}
