...
export class SupabasePersistence {
    ...
    for (let attempt = 0; attempt < (retryable ? 3 : 1); attempt += 1) {
      try {
        ...
        if (!response.ok) {
          ...
          if (retryable && transientHydrationError(error) && attempt < 2) { lastError=error; await sleep(500 * 2 ** attempt); continue; }
          throw error;
        }
  ...
  async rows(table,{select="*",order="",filters={},maxRows=10000,pageSize=1000}={}) {
    ...
    for(let offset=0;offset<maxRows;offset+=size){
      const params=new URLSearchParams({select}); if(order)params.set("order",order); for(const[key,value]of Object.entries(filters))params.set(key,value);
      ...
      if(!Array.isArray(page))throw Error(`Unexpected Supabase ${table} response`); out.push(...page); if(page.length<wanted)break;
    }
    return out;
  }
  async observationRows({maxRows=100000,pageSize=500}={}) {
    const out=[],size=Math.max(1,Math.min(1000,pageSize)); let cursor=null;
    while(out.length<maxRows){const wanted=Math.min(size,maxRows-out.length),params=new URLSearchParams({select:"*",order:"at.desc,id.desc",limit:String(wanted)});if(cursor)params.set("or",`(at.lt.${cursor.at},and(at.eq.${cursor.at},id.lt.${cursor.id}))`);const page=await this.request(`${TABLES.observations}?${params.toString()}`);if(!Array.isArray(page))throw Error("Unexpected Supabase observations response");out.push(...page);if(page.length<wanted)break;const last=page.at(-1);if(!last||last.at==null||!last.id)break;cursor={at:last.at,id:last.id};} return out;
  }
  async eventRows({maxRows=10000,pageSize=500}={}) {
    const out=[],size=Math.max(1,Math.min(1000,pageSize));let beforeSeq=null;
    while(out.length<maxRows){const wanted=Math.min(size,maxRows-out.length),params=new URLSearchParams({select:"*",order:"seq.desc",limit:String(wanted)});if(beforeSeq!==null)params.set("seq",`lt.${beforeSeq}`);const page=await this.request(`${TABLES.events}?${params.toString()}`);if(!Array.isArray(page))throw Error("Unexpected Supabase events response");out.push(...page);if(page.length<wanted)break;const last=page.at(-1);if(last?.seq==null)break;beforeSeq=last.seq;}return out;
  }
  async entityRows(kind,{maxRows=10000,pageSize=500}={}) {
    const out=[],size=Math.max(1,Math.min(1000,pageSize));let cursor=null;
    while(out.length<maxRows){const wanted=Math.min(size,maxRows-out.length),params=new URLSearchParams({select:"*",kind:`eq.${kind}`,order:"at.desc,id.desc",limit:String(wanted)});if(cursor)params.set("or",`(at.lt.${cursor.at},and(at.eq.${cursor.at},id.lt.${cursor.id}))`);const page=await this.request(`${TABLES.entities}?${params.toString()}`);if(!Array.isArray(page))throw Error(`Unexpected Supabase ${kind} entities response`);out.push(...page);if(page.length<wanted)break;const last=page.at(-1);if(!last||last.at==null||!last.id)break;cursor={at:last.at,id:last.id};}return out;
  }
  ...
  async pruneJobs(now=Date.now()){const cutoff=now-ACTIVE_JOB_MAX_AGE_MS;await Promise.all([this.deleteJobs({state:"in.(done,dead)"}),this.deleteJobs({state:"eq.pending",available:`lt.${cutoff}`}),this.deleteJobs({state:"eq.running",lease_until:`lt.${now}`})]);}
  async upsert(table,row,conflict){if(table==="jobs"&&!Array.isArray(row)&&TERMINAL_JOB_STATES.has(row?.state)){await this.deleteJobs({id:`eq.${row.id}`});return;}const suffix=conflict?`?on_conflict=${encodeURIComponent(conflict)}`:"";await this.request(TABLES[table]+suffix,{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify(row)});}
  ...
  async hydrateOnce(store) {
    const now=Date.now(),cutoff=now-ACTIVE_JOB_MAX_AGE_MS; await this.pruneJobs(now);
