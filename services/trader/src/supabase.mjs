...
export class SupabasePersistence {
  ...
  async upsert(table,row,conflict){const suffix=conflict?`?on_conflict=${encodeURIComponent(conflict)}`:"";await this.request(TABLES[table]+suffix,{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify(row)});}
  ...
  async hydrate(store) {
    ...
    // Previous 25k/20k caps silently changed the strategy after every restart by dropping
    // older evidence. Keep bounded hydration, but high enough to preserve the working history.
